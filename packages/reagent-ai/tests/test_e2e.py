...
def _run_agent(session: reagent_ai.Session) -> None:
...
def test_record_and_assert(tmp_path: Path) -> None:
    with reagent_ai.session("e2e_test", trace_dir=str(tmp_path)) as s:
        _run_agent(s)
...
def test_golden_baseline_match(tmp_path: Path) -> None:
    # Record golden
    with reagent_ai.session("e2e_golden", golden=True, trace_dir=str(tmp_path)) as s:
        _run_agent(s)

    # Record actual — same behavior
    with reagent_ai.session("e2e_golden", trace_dir=str(tmp_path)) as s2:
        _run_agent(s2)
...
def test_golden_baseline_divergence(tmp_path: Path) -> None:
    # Record golden
    with reagent_ai.session("e2e_div", golden=True, trace_dir=str(tmp_path)) as s:
    ...
    with reagent_ai.session("e2e_div", trace_dir=str(tmp_path)) as s2:
...
def test_trace_file_persisted(tmp_path: Path) -> None:
    with reagent_ai.session("persist_test", trace_dir=str(tmp_path)) as s:
